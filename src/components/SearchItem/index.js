function SearchItem({ title = false, searched = false }) {
  return (
    <div className='search-item w-full flex items-center py-1 hover:bg-[#f2f2f2] overflow-hidden line-clamp-1'>
      <div className='px-1 md:pl-[14px] md:pr-[12px]'>
        {searched ? (
          'cái này code icon search'
        ) : (
      ...
      <div className='title-search line-clamp-1 font-medium select-none cursor-pointer'>
        {title}
      </div>
    </div>
  )
